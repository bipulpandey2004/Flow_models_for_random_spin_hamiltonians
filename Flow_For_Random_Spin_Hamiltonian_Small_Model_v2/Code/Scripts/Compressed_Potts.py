...
class Compressed_Potts:
    ...
    ####------------------------------------ Outflow -------------------------------------------------------
    def Layered_MPF_flow_Gradient_Single_Pass(self, Data_Properties, method , power = 1):
        """
        ...
        for hsh_val, prop in Properties.items():
            ...
            if method ==1:
                ...
                # Exponential
                Factor = 1
                p_i_power = np.exp(Multiplicity/Total_data_size) -1
                self.method = "MPF Exponential"

            else:
                # Outflow with power
                Factor = 1
                self.method = "MPF with entropy gap correction"
                p_i_power = 1.5*(Multiplicity < 1)**(power)
    ...
    def Exact_Min_Outflow_Single_Iteration_Parameter_Update(self, Data_Properties, epsilon, method = 1, power = 1):
        ...
        # Compute gradients
        Weight_Gradient_contribution, Bias_Gradient_contribution, Total_Outflow = self.Layered_MPF_flow_Gradient_Single_Pass(Data_Properties, method=method, power = power)
